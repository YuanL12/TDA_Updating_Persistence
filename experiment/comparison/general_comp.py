# This script will combine computation on all dataset together 
...
# this function will return a line of record in csv file
# X is original dataset, X2 is the one need to be computed on
def line_record(dataset_name, X, X2, d): 
    # write each row
    record_line = [] # record for each line
    record_line.append(dataset_name)
...
# Now ready to run
# write into files
header = ['dataset','standard_basis','standard_no_basis','clearing','compression','update',
...
with open(filename, 'a', encoding='UTF8', newline='') as f:
    ...
    random_indices = np.random.choice(n_total_rows, size=20, replace=False)
    X = X[random_indices, :]
    ...
    for i in range(n_repeats):
        ...
        # write each row
        d = 3
        record_line = line_record(data_name, X, X2, d)
